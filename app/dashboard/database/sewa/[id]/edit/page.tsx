...
export default function EditHargaSewaPage() {
  const router = useRouter()
  const params = useParams()
  const id = params.id as string

  const [form, setForm] = useState({ nama_item: '', satuan: '', harga: '', kategori: '', berlaku_mulai: '', catatan: '' })
  ...
  useEffect(() => {
    async function fetch() {
      ...
      const { data, error } = await supabase.from('harga_sewa').select('*').eq('id', id).single()
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setError('')
    if (!form.nama_item.trim()) return setError('Nama item wajib diisi.')

    setSaving(true)
    const supabase = createClient()
    const { error } = await supabase.from('harga_sewa').update({
    ...
    router.push('/dashboard/database/sewa')
  }
  ...
  return (
    <div style={{ maxWidth: '560px' }}>
      <div style={{ marginBottom: '28px' }}>
        <button onClick={() => router.push('/dashboard/database/sewa')}
          style={{ background: 'none', border: 'none', color: SECONDARY, fontSize: '12px', cursor: 'pointer', padding: 0, marginBottom: '12px' }}>
          ← Kembali
        </button>
        <h1 style={{ fontSize: '22px', fontWeight: '600', color: NAVY, margin: '0 0 6px 0' }}>Edit Harga Sewa</h1>
        <p style={{ fontSize: '13px', color: SECONDARY, margin: 0 }}>Ubah detail harga sewa</p>
      </div>
      ...
      <div style={{ backgroundColor: CARD_BG, border: `1px solid rgba(13,46,66,0.15)`, borderRadius: '12px', padding: '32px', boxShadow: '0 1px 3px rgba(13,46,66,0.06)' }}>
        <form onSubmit={handleSubmit}>
          <Field label="Nama Item *">
            <input name="nama_item" value={form.nama_item} onChange={handleChange}
              placeholder="Contoh: Scaffolding, Excavator PC200" style={inputStyle} />
          </Field>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            ...
            <Field label="Satuan">
              <input name="satuan" value={form.satuan} onChange={handleChange}
                placeholder="Contoh: hari, bulan, unit" style={inputStyle} />
            </Field>
          </div>
          ...
          <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
            ...
            <button type="button" onClick={() => router.push('/dashboard/database/sewa')}
