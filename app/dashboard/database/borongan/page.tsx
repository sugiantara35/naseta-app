...
const GOLD = '#D4AF37'
const BASE_PATH = '/dashboard/database/borongan'
...
async function deleteItem(id: string) {
  ...
  await supabase.from('harga_borongan').delete().eq('id', id)
  revalidatePath('/dashboard/database/borongan')
}
...
export default async function HargaBoronganPage({
  searchParams,
}: {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: '600', color: NAVY, margin: '0 0 6px 0' }}>Harga Borongan</h1>
          <p style={{ fontSize: '13px', color: SECONDARY, margin: 0 }}>{items.length} item{activeKategori ? ` · ${activeKategori}` : ' terdaftar'}</p>
        </div>
      ...
      <div style={{ backgroundColor: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: '12px', overflowX: 'auto', boxShadow: '0 1px 3px rgba(13,46,66,0.06)' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '680px' }}>
          ...
          <tbody>
            {!items || items.length === 0 ? (
            ...
            ) : items.map((item: HargaBorongan, i: number) => (
              <tr key={item.id} style={{ borderBottom: i < items.length - 1 ? `1px solid ${BORDER}` : 'none' }}>
                ...
                <td style={{ ...tdStyle, color: SECONDARY, maxWidth: '200px' }}>
                  <span style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                    {item.keterangan_lingkup ?? '—'}
                  </span>
                </td>
                <td style={{ ...tdStyle, color: SECONDARY }}>{item.berlaku_mulai ?? '—'}</td>
                <td style={tdStyle}>
                  <div style={{ display: 'flex', gap: '8px' }}>
                    <Link href={`/dashboard/database/borongan/${item.id}/edit`} style={{
