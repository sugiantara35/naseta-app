...
export default function EditHargaBoronganPage() {
  ...
  useEffect(() => {
    async function fetch() {
      ...
      const { data, error } = await supabase.from('harga_borongan').select('*').eq('id', id).single()
      ...
      setForm({
        ...
        keterangan_lingkup: data.keterangan_lingkup ?? '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true)
    const supabase = createClient()
    const { error } = await supabase.from('harga_borongan').update({
    ...
    router.push('/dashboard/database/borongan')
  }
  ...
  return (
    <div style={{ maxWidth: '560px' }}>
      <div style={{ marginBottom: '28px' }}>
        <button onClick={() => router.push('/dashboard/database/borongan')}
          style={{ background: 'none', border: 'none', color: SECONDARY, fontSize: '12px', cursor: 'pointer', padding: 0, marginBottom: '12px' }}>
          ← Kembali
        </button>
        <h1 style={{ fontSize: '22px', fontWeight: '600', color: NAVY, margin: '0 0 6px 0' }}>Edit Harga Borongan</h1>
        <p style={{ fontSize: '13px', color: SECONDARY, margin: 0 }}>Ubah detail harga borongan</p>
      </div>
      ...
      <div style={{ backgroundColor: CARD_BG, border: `1px solid rgba(13,46,66,0.15)`, borderRadius: '12px', padding: '32px', boxShadow: '0 1px 3px rgba(13,46,66,0.06)' }}>
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            <Field label="Satuan">
              <input name="satuan" value={form.satuan} onChange={handleChange}
                placeholder="Contoh: m², ls, unit" style={inputStyle} />
            </Field>
          ...
          <Field label="Keterangan Lingkup">
            <textarea name="keterangan_lingkup" value={form.keterangan_lingkup} onChange={handleChange}
              placeholder="Deskripsi lingkup pekerjaan yang dicakup..." rows={3}
          ...
          <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
            ...
            <button type="button" onClick={() => router.push('/dashboard/database/borongan')}
